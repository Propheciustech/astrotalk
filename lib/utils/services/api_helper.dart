...
//call Astrologer request API
  Future<dynamic> sendAstrologerCallRequest(
      int astrologerId, bool isFreeSession) async {
    try {
      final response = await http.post(
...
//=------------------------------chat----------------->
  ...
  Future<dynamic> acceptChat(int chatId) async {
    try {
      final response = await http.post(Uri.parse('$baseUrl/chatRequest/accept'),
          headers: await global.getApiHeaders(true),
  ...
  Future<dynamic> getKundliBasicPanchangDetails(
    ...
    try {
      ...
      print("pachaaaaaaaaaaaaaaaaaaaang");
  ...
  Future<dynamic> getMatching(
    ...
    try {
      final response = await http.post(
        ...
        headers: {
          "x-api-key": 'twp9tWVO0Z6BBowRGa5Es4BXbxySm4s8YbDZAWed',
          "Content-Type": 'application/json'
        },
      );
  ...
  ''';
        recordList = json.decode(jsonString);
        dynamic apiresp = json.decode(response.body);
        print(apiresp['output']);
        recordList['varna']['total_points'] =
            apiresp['output']['varna_kootam']['out_of'];
        recordList['varna']['received_points'] =
            apiresp['output']['varna_kootam']['score'];

        recordList['vashya']['total_points'] =
            apiresp['output']['vasya_kootam']['out_of'];
        recordList['vashya']['received_points'] =
            apiresp['output']['vasya_kootam']['score'];

        recordList['tara']['total_points'] =
            apiresp['output']['tara_kootam']['out_of'];
        recordList['tara']['received_points'] =
            apiresp['output']['tara_kootam']['score'];

        recordList['yoni']['total_points'] =
            apiresp['output']['yoni_kootam']['out_of'];
        recordList['yoni']['received_points'] =
            apiresp['output']['yoni_kootam']['score'];

        recordList['maitri']['total_points'] =
            apiresp['output']['graha_maitri_kootam']['out_of'];
        recordList['maitri']['received_points'] =
            apiresp['output']['graha_maitri_kootam']['score'];

        recordList['gan']['total_points'] =
            apiresp['output']['gana_kootam']['out_of'];
        recordList['gan']['received_points'] =
            apiresp['output']['gana_kootam']['score'];

        recordList['bhakut']['total_points'] =
            apiresp['output']['rasi_kootam']['out_of'];
        recordList['bhakut']['received_points'] =
            apiresp['output']['rasi_kootam']['score'];

        recordList['nadi']['total_points'] =
            apiresp['output']['nadi_kootam']['out_of'];
        recordList['nadi']['received_points'] =
            apiresp['output']['nadi_kootam']['score'];

        recordList['total']['total_points'] = apiresp['output']['out_of'];
        recordList['total']['received_points'] =
            apiresp['output']['total_score'];
  ...
  Future<dynamic> getHoroscope({int? horoscopeSignId}) async {
    print("fayyyyyyyysssssssssssss");
    print(horoscopeSignId);
    if (horoscopeSignId == null) horoscopeSignId = 10;
    print(horoscopeSignId);
    try {
      final response = await http.post(
      ...
      print("fayyyyyyyysssssssssssss");
      print(json.decode(response.body)['recordList']);
      dynamic recordList;
      if (response.statusCode == 200) {
        recordList = json.decode(response.body)['recordList'];
      } else {
...
//report and block astrologer profile review
  ...
  Future getZodiacProfileImg() async {
    try {
      ...
      print("Exception: api_helper.dart - getZodiacProfileImg(): " +
          e.toString());
    }
  }
