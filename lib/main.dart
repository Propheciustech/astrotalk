...
Future<void> _firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  ...
  if (global.sp!.getString("currentUser") != null) {
    ...
    if (message.notification!.title ==
      ...
      print(
          '_firebaseMessagingBackgroundHandler For starting the timer in other audions for video and audio');
      ...
      if (liveController.isImInLive == true) {
        ...
        print("liveController.waitList" +
            liveController.waitList[0].id.toString());
        int index5 = liveController.waitList
            .indexWhere((element) => element.id == waitListId);
        if (index5 != -1) {
          print("joined waitlist user time = " +
              liveController.waitList[index5].time.toString());
