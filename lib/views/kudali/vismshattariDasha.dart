          ...
          children: [
            (kundliController.vimshattariList!.isNotEmpty)
                ? SingleChildScrollView(
                    ...
                    child: Row(
                      children: [
                        (kundliController.vimshattariList!.length >= 1)
                            ? stapper(controller: kundliController, index: 0)
                            : SizedBox(),
                        (kundliController.vimshattariList!.length >= 2)
                            ? stapper(controller: kundliController, index: 1)
                            : SizedBox(),
                        (kundliController.vimshattariList!.length >= 3)
                            ? stapper(controller: kundliController, index: 2)
                            : SizedBox(),
                        (kundliController.vimshattariList!.length >= 4)
                            ? stapper(controller: kundliController, index: 3)
                            : SizedBox(),
                        (kundliController.vimshattariList!.length >= 5)
                            ? stapper(controller: kundliController, index: 4)
                            : SizedBox(),
                        (kundliController.vimshattariList!.length >= 6)
                            ? stapper(controller: kundliController, index: 5)
                            : SizedBox(),
                      ],
                    ),
                  )
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        ListView.builder(
                            shrinkWrap: true,
                            itemCount:
                                kundliController.vimshattariList!.last.length,
                            itemBuilder: ((context, index) {
                              return GestureDetector(
                                onTap: () async {
                                  if (kundliController.vimshattariList!.length <
                                      6) {
                                    ...
                                    kundliController.prefix =
                                        "${kundliController.prefix}${kundliController.vimshattariList!.last[index].planet!.substring(0, 2).toUpperCase()}-";
                                    if (kundliController
                                            .vimshattariList!.length ==
                                        1) {
                                      await kundliController
                                          .getAntardashaDetail(
                                              antarName: kundliController
                                                  .vimshattariList![index]
                                                  .last
                                                  .planet,
                                              day: formattedDay,
                                              hour: formattedHour,
                                      ...
                                      kundliController.update();
                                            ...
                                            .vimshattariList!.length ==
                                        4) {
                                      await kundliController.getPranaDetail(
                                          day: formattedDay,
                                          hour: formattedHour,
                                          min: formattedMint,
                                          month: formattedMonth,
